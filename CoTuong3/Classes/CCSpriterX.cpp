...
namespace SCMLHelper
{
	...
	void Animation::Update(float dt)
	{
		...
		if (mTimer >= mLength)
		{
			...
			if (m_Delegate)
			{
				m_Delegate->spriterRunDone(this);
			}
    ...
    bool  Entity::playAnimation(const char *animationName) {
        std::string aName(animationName);
        
        for (int i = 0; i < mAnimations.size(); ++i) {
            Animation *animation = mAnimations[i];
            if (animation->getName().compare(aName) == 0) {
                mCurrAnimation = i;
                animation->Restart();
                return true;
                break;
...
void CCSpriterX::playAnimation(const char *animationName) {
    for (int i = 0; i < mEntities.size(); ++i) {
        if (mEntities[i]->playAnimation(animationName)) {
			mEntities[i]->getCurrentAnimation()->setDelegate(m_Delegate);
            mCurrEntity = i;
            break;
        }
    }
...
std::string CCSpriterX::getCurrAnimationName(){
    return mEntities[mCurrEntity]->getCurrentAnimation()->getName();
}
...
bool CCSpriterX::initWithFile(const char *filename)
{
	...
	const char* path;
	std::string _path = CCFileUtils::sharedFileUtils()->fullPathForFilename(filename);
	path=_path.data();
...
void CCSpriterX::setDelegate(SpriterXDelegate* delegated)
{
